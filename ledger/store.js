...
//
// 五句规则（KAIROS-CONTRACT §六）里，账本侧这四句由本文件负责：
//   规则 1 锁全程   → withLedgerLock：读—改—写整个事务在锁内，写完即放
//   规则 2 手动改不被覆盖 → beingWrite 跳过 last_writer=human 的字段
//   规则 4 先文件后消息 → commitThenNotify：先落盘，成功后才发消息，发失败不回滚
//   规则 3 快照落盘   → 心跳自己的事，路径在 paths.HEARTBEAT_SNAPSHOT
//
...
/** 业务字段。Swift 侧 KairosField.business 必须逐字相同（规则 2 按字段比对，名字对不上就等于没保护）。
 *
 ...
 *    links   规则指派它链另一行待办，类型却是 {label,url}（一个网址），对不上，也没有 UI。
 */
...
/** 对方长得对不对。**不合格就整个字段作废**（和值集同一条规则：作废那一个字段，不牵连整条）。
 ...
 *  是整个快照。和 counterpart / thread 同一条规则：不合格就整个字段作废，不牵连整条。 */
 ...
 *  炉火有很多个，得说清是哪一个；篝火是全局的、只有一个，和待办一样裸写。 */
 ...
 * v2.3 规则 2 的后半句：「心跳写人改过的字段需要新的用户信号作依据」——
