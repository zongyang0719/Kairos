...
// 五句规则由 ledger/store.js 强制执行，改不动的字段会明说跳过，不会安静盖掉。
//
// 用法：
...
switch (command) {
  ...
  case 'create': {
    ...
    if (typeof flags.state === 'string') die(STATE_GONE);
    for (const key of ['tier', 'status', 'project', 'source', 'excerpt', 'summary', 'brief', 'reason', 'ask', 'evidence']) {
      if (typeof flags[key] === 'string') fields[key] = flags[key];
    }
    if (typeof fields.evidence === 'string') {
      try { fields.evidence = JSON.parse(fields.evidence); }
      catch { die(EVIDENCE_HELP); }
    }
    if (fields.evidence !== undefined && !store.isValidEvidence(fields.evidence)) die(EVIDENCE_HELP);
    // 与 set 同一道闸：了结（closed）不可逆，建行就关必须带证据。
    if (fields.status === store.CLOSED && !(Array.isArray(fields.evidence) && fields.evidence.length)) {
      die('了结要有证据（不可逆：盖掉原状态，重开只回 todo）。给 --evidence 一条真源链接，或先建 todo 再走 set。');
    }
