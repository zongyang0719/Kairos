...
struct ItemDetailView: View {
    ...
    var body: some View {
        if let item {
            ...
            // 开着这一屏时有了结果（答完、没接住、没发出去——不一定多出一条消息），也算看到了。
    ...
    private func bubble(_ message: KairosRoomMessage, item: KairosItem) -> some View {
        ...
        case KairosMessageOrigin.user:
            VStack(alignment: .trailing, spacing: 4) {
                ...
                // 还在排队的那句没轮到，不是没发出去——以前这里一样画成红字。
                // being 在忙时它要等上一阵，一直挂着「没发出去」就是在说假话。
                if message.queued {
                    Text(store.queuedCaption(for: message))
                        .font(.caption)
                        .foregroundStyle(.tertiary)
                } else if message.unanswered {
                    // 送到了但没答完：在这间再问一次，不然 being 会在别的房间里答这句。
                    Button {
                    ...
                    .buttonStyle(.plain)
                } else if !message.delivered, !store.sendingMessageIDs.contains(message.id) {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            Group {
                if settled.isEmpty {
                    ...
                    richText(settled, streaming: true)
                        .font(.callout)
                }
            }
            .padding(.horizontal, 14)
    ...
    private func composer(_ item: KairosItem) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            // 只画这一条自己的：别的待办发失败不关这里的事，退出去再点回来也还在。
    ...
    /// `streaming`：正文还在流。真时**始终走下面那条 VStack**——第一块之外再出来一块时，
    /// 不会从「一个 Text」整个换成「一个 VStack」，那一换是拆了重建，屏幕上闪一下。
    @ViewBuilder
    private func richText(_ text: String, streaming: Bool = false) -> some View {
