...
///
/// 关联之后规矩一个字没变（v2.3 §五 + 2026-09-07 多端写入）：
...
enum KairosLedgerLocation {
    ...
    /// 邮箱镜像顺路带过去——它和账本住同一个文件夹（`KairosMailStorage`），
    /// 位置一换不带上，Inbox 就会突然变成「还没接通」。
    ///
    /// **只在目标没有的时候带**：那边有一份的话，那份多半比这份新（being 在那边跑过 mail-sync），
    /// 拿旧的盖上去等于把已经收到的信抹回去。镜像本来就是可再生的，宁可少动。
    static func carryMailbox(from source: URL, to target: URL) {
        let name = KairosMailFiles.mailboxName
        ...
        guard FileManager.default.fileExists(atPath: from.path),
              ...
              let data = try? Data(contentsOf: from) else { return }
...
/// 换位置时把两边的账本并起来。
///
/// 和 `KairosLedgerMerge.rebase`（规矩 1：拿到锁之后把自己这段改动叠到盘上最新那份）
...
enum KairosLedgerUnion {
    ...
    private static func inheriting(_ winner: KairosItem, from loser: KairosItem) -> KairosItem {
        ...
        // 规矩 2 宁可挡住 being 的一次自动写入，也不能把人的决定悄悄交回去。
