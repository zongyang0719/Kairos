...
struct KairosRoomMessage: Codable, Hashable, Identifiable {
    ...
    /// 规则 4：先文件后消息。消息发失败**不回滚文件**，留在这儿标记未送达，
    /// 下次连上补发；补不上也不致命，心跳 diff 自愈会补。
    var delivered: Bool
    /// 还在发送队列里排着，轮不到它开口。
    ///
    /// 「排队中」和「没发出去」是两回事：气泡是按下发送那一刻就画出来的，但一条一条来
    /// （见 `KairosStore.speak`），前面那句的整趟回复没收完就不发下一句。这段时间里
    /// 照着 `delivered == false` 画红字，人会看见「没发出去，点一下重发」——那句话此刻是假的。
    var queued: Bool
    /// 送到了，但 being 没答完：回复半路断了，它那边也没留下这句的回复（`KairosStore.resumeStream`）。
    ///
    /// 和「没发出去」不是一回事——话它听到了。可不管它，下一间房一开口，being 新起的那口气
    /// 会先把这句答了，回复落进别的房间。所以给人一个「再问一次」，在**这间**问。
    var unanswered: Bool
    ...
    enum CodingKeys: String, CodingKey {
        case id, itemID, origin, weight, text, createdAt, delivered, queued, unanswered
    }

    /// **必须手写**，同 `KairosRoom.init(from:)` 那段注释：合成的解码器不认属性默认值，
    /// 老的 `rooms.json` 没有 `queued` 这个键，合成解码器会抛 keyNotFound，
    /// `KairosRooms.load()` 读不出就回 `.empty`——所有房间日志静默清空。
    init(from decoder: Decoder) throws {
        let box = try decoder.container(keyedBy: CodingKeys.self)
        id = try box.decode(String.self, forKey: .id)
        itemID = try box.decode(String.self, forKey: .itemID)
        origin = try box.decode(String.self, forKey: .origin)
        weight = try box.decode(String.self, forKey: .weight)
        text = try box.decode(String.self, forKey: .text)
        createdAt = try box.decode(String.self, forKey: .createdAt)
        delivered = try box.decodeIfPresent(Bool.self, forKey: .delivered) ?? false
        queued = try box.decodeIfPresent(Bool.self, forKey: .queued) ?? false
        unanswered = try box.decodeIfPresent(Bool.self, forKey: .unanswered) ?? false
    }
}
...
struct KairosRoom: Codable, Hashable {
    ...
    /// being 那头的动静，一样一拍：回了一段话（`KairosRooms.append`）、这一口气收了
    /// （答完、没接住回复、没发出去——`KairosStore.noteTurnsEnded`）。
    /// 和 `seenBeats` 一起说「有没有你还没看的结果」，单子上那一行据此亮点（`KairosRoomPulse`）。
    ///
    /// 用计数不用时间戳：`readUpTo` 只到秒，人前脚刚离开这间房、回复后脚落在同一秒里，
    /// 就被算成看过了；计数没有这条缝。
    var beats: Int = 0
    /// 人看到第几拍了。点进这间房（`markRead`）就追平。
    var seenBeats: Int = 0

    /// 规则 5：只数 ask 级，且只数别人说的。routine 再多也不亮。
    ...
    init(from decoder: Decoder) throws {
        ...
        // 老文件没有这两个数：当 0，升级那一刻不会冒出一片旧的点。
        beats = try box.decodeIfPresent(Int.self, forKey: .beats) ?? 0
        seenBeats = try box.decodeIfPresent(Int.self, forKey: .seenBeats) ?? 0
    }

    var hasUnreadAsk: Bool { unreadAskCount > 0 }

    var hasUnseenBeat: Bool { beats > seenBeats }

    /// 人在这间房里说的最后一句。
    var lastUserMessage: KairosRoomMessage? {
        messages.last { $0.origin == KairosMessageOrigin.user }
    }
...
struct KairosRooms: Codable, Hashable {
    ...
    /// 机器来回（账本）的固定房间号，`来回名 -> session_id`。
    ///
    /// BeingDesktop 的做法：client 自己生成 UUID、本地存、每次请求都带上。信息不串的
    /// 根子在这里——账本的话永远落账本那间，进不了任何一条待办的房间，也进不了
    /// 人类和 being 的主对话。以前机器流量发的是 `session_id: nil`，全落进主对话当噪音。
    ...
    mutating func append(_ message: KairosRoomMessage) {
        ...
        if let index = room.messages.firstIndex(where: { $0.id == message.id }) {
            ...
            // 回话一落进来就记一拍，不等这口气收：202 之后晚到的后半段，是在「在答」收起之后才来的。
            if message.origin == KairosMessageOrigin.being { room.beats += 1 }
        }
    ...
    mutating func markDelivered(_ messageID: String, in itemID: String) {
        ...
        room.messages[index].queued = false
        rooms[itemID] = room
    }

    /// 送到了但没答完（见 `KairosRoomMessage.unanswered`）。
    mutating func markUnanswered(_ messageID: String, in itemID: String) {
        guard var room = rooms[itemID],
              let index = room.messages.firstIndex(where: { $0.id == messageID }) else { return }
        room.messages[index].unanswered = true
        rooms[itemID] = room
    }

    /// 轮到它了：从「排队中」进入「正在发」。发的结果由 `markDelivered` 或红字气泡说。
    mutating func markDequeued(_ messageID: String, in itemID: String) {
        guard var room = rooms[itemID],
              let index = room.messages.firstIndex(where: { $0.id == messageID }) else { return }
        room.messages[index].queued = false
        rooms[itemID] = room
    ...
    /// 队列只活在内存里（`KairosStore` 那根泵），app 一关就没了。所以读盘时把留在
    /// 盘上的「排队中」全落成「没发出去」——没人会再替它们开口，挂着排队标记是假话，
    /// 落成没发出去人还能点重发。
    ///
    /// 这也是一个结果，记一拍：不然单子上那一行什么都不说，人会以为那几句早就发出去了。
    mutating func clearStaleQueue() {
        for (itemID, var room) in rooms {
            guard room.messages.contains(where: { $0.queued }) else { continue }
            for index in room.messages.indices where room.messages[index].queued {
                room.messages[index].queued = false
            }
            room.beats += 1
            rooms[itemID] = room
    ...
    /// 这间房有了一个结果（见 `KairosRoom.beats`）。房间不存在就不记——刚删掉的待办、
    /// 一次性的对话，不为它们造一间空房。回 true = 记上了。
    @discardableResult
    mutating func markBeat(_ itemID: String) -> Bool {
        guard var room = rooms[itemID] else { return false }
        room.beats += 1
        rooms[itemID] = room
        return true
    }

    func hasUnseenBeat(_ itemID: String) -> Bool { room(itemID).hasUnseenBeat }
    ...
    static func load() -> KairosRooms {
        guard let data = try? Data(contentsOf: KairosFiles.rooms),
              var value = try? JSONDecoder().decode(KairosRooms.self, from: data) else {
            return .empty
        }
        value.clearStaleQueue()
        return value
    }
