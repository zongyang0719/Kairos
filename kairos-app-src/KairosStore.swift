...
enum KairosFiles {
    ...
    // MARK: - 账本事务（v2.3 规矩 1：锁全程）

    /// 读—改—写整个事务在锁内完成，写完即放。
    ///
    /// 注意 `body` 拿到的是**锁内刚从盘上读回来的**账本，不是调用方手里那份可能已经
    /// 过期的内存副本——这正是规矩 1 的要害：只锁「写那一下」的话，两个写入者仍然
...
final class KairosStore: ObservableObject {
    ...
    /// 最近一次没发出去的原因。就地显示在对话页，不弹全局弹窗——弹窗会把人正在看的
    /// 那一屏顶掉，而且关掉就找不回来了。
    @Published var sendError: String?
    /// 房间（渠道）的本机状态：房间日志、已读游标、没送达的消息。
    /// 派生文件，不进 iCloud——房间整个删掉，账本 diff 自愈，什么都不少。
    @Published var rooms = KairosRooms.empty
    /// Beings Town 邮局：being 写的镜像 + 这台设备还没被取走的草稿 + 本机已读游标。
    /// Kairos 够不着 Town（IP 信任只认 being 的 Heart），全靠 being 代收代发——见 KairosMail.swift。
    @Published private(set) var mailbox = KairosMailbox.empty
    @Published private(set) var mailDrafts: [KairosMailDraft] = []
    @Published private(set) var mailRead = KairosMailReadCursor()
    ...
    /// 向 being 要 BEING-RULES 的状态（按当前连接键，见 `beingRulesConnectionKey`）。
...
#endif
    private let mailStorage = KairosMailStorage()
    ...
    init() {
        ...
        mailRead = KairosMailReadCursor.load()
        loadSnapshot()
    ...
    private var mailboxWatch: DispatchSourceFileSystemObject?

    /// 也盯着账本所在的目录——being 跑完 `mail-sync` 是原子换掉 mailbox.json，目录会有一次
    /// 写事件。不盯的话 Inbox 那一屏得等人按 ⌘R 才更新，而那一屏恰恰是等信的地方。
    private func startWatchingMailbox() {
        mailboxWatch?.cancel()
        mailboxWatch = nil
        let descriptor = open(KairosFiles.dataDirectory.path, O_EVTONLY)
        guard descriptor >= 0 else { return }
        let source = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: descriptor,
            eventMask: [.write],
    ...
    ///
    /// 「跟Heart说这件事」「记一条，或者问Soul」——中英文贴在一起，字挤成一团，
    ...
    /// 「落款是 Heart：信由 ta 代收代发」——标点前面不留空格，那是中文排版的硬规矩。
    ...
    /// 侧栏该显示的是「有多少条在等你」——只数 ask 级未读（规矩 5）。
...
// 围着 `workspace.membership` 长出来的那一圈 2026-09-11 整块撤掉：
    ...
    func checkBeing() async {
        ...
        do {
            ...
            // 名字：设置里填了就用填的，没填才记它自报的（界面上叫 Soul，不是它自报的那个名字）。
    ...
    /// 上次没送达的房间消息（规矩 4：消息发失败不回滚文件，留着下次补）。
    func refresh() async {
...
#endif
        reloadFromDisk()
        // 邮箱和账本住同一个文件夹，刷新一次两个都读——邮件的新鲜度跟着刷新走，
        // 不另开一条轮询（Kairos 是回合制的，不该有第二个心跳）。
        reloadMail()
        ingestMessages()
        await flushOutbox()
    }
    ...
    func linkLedger(to folder: URL) -> Bool {
        ...
        KairosLedgerLocation.carryMailbox(from: currentURL.deletingLastPathComponent(), to: target)
    ...
    // MARK: - 邮局（Beings Town DM）
    //
    // Kairos 不直接跟 Town 说话：Town 的 DM 接口按 IP 信任 being 的 Heart，
    // 从这台机器打过去是 401。being 代收代发，两边靠文件交接（KairosMail.swift 顶上写了全部理由）。
    // 所以这里只有三个动作：把镜像读回来、把草稿写下去、记住读到哪儿了。

    /// 把镜像和自己的草稿箱从盘上读回来。刷新、启动、发完信之后都走这条。
    func reloadMail() {
        let mirror = mailStorage.readMailbox()
        if let mirror { mailbox = mirror }

        guard var outbox = mailStorage.readOutbox() else {
            mailDrafts = []
            return
        }
        // 水位说「你到第 N 封为止我都发过了」，那 N 封就可以删了。
        // 我们是这个文件的唯一写者，删自己的东西不会跟谁打架。
        let watermark = mailbox.outboxWatermark[KairosDevice.id] ?? 0
        let before = outbox.drafts.count
        outbox.prune(sentThrough: watermark)
        if outbox.drafts.count != before { mailStorage.writeOutbox(outbox) }
        mailDrafts = outbox.drafts
        scanIncomingMessagesForBeingRules()
    }

    /// 界面要的东西：按人分组的对话。
    var mailThreads: [KairosMailThread] {
        KairosMailAssembly.threads(
            mailbox: mailbox,
            drafts: mailDrafts,
            cursor: mailRead,
            me: mailbox.beingID
    ...
    /// 未读只数收到的信。发出去的不管成败都不算未读——那是状态，不是「有人找你」。
    var mailUnreadCount: Int {
        KairosMailAssembly.unreadCount(mailbox: mailbox, cursor: mailRead)
    }
    ...
    /// 邮箱有没有配起来：镜像文件在不在。不在就说明 being 还没跑过 mail-sync，
    /// 界面要说清楚是「还没接通」而不是「没有信」——这两件事看起来一样，处理方式完全不同。
    var mailIsWired: Bool { mailbox.syncedAt != nil }

    /// 叫 being 去一趟邮局。
    ///
    /// Kairos 自己够不着 Town（IP 信任只认 being 的 Heart），所以在这儿，
    /// 「刷新收件箱」这个动作**只能是请人去，不能是自己去**。下拉刷新只是重读盘上的
    /// 镜像——镜像新不新，取决于 being 上一次去邮局是什么时候。
    ///
    /// 这是对话通道在整条邮件流程里唯一该出现的地方：一句人话，人自己按的按钮触发的。
    /// 机器对机器的那部分（取信、投递、回执）全部走文件，不经过这里。
    @discardableResult
    func nudgeMailRound() async -> Bool {
        // 连着共享文件夹时，`mailbox.json` 的写者是 being：这边只喊一嗓子，
        // 让它按操作卡跑 CLI，然后读盘。自己动那个文件就是两个写者。
        guard mailStorage.ownsMailbox else {
            let reply = await speak(
                "去一趟邮局：按 BEING-RULES.md 的「邮局」那节跑一轮——"
                    + "把 Town 的收发件箱灌进 mailbox.json，再看看 mail-pending 里有没有我托你发的信。",
                in: nil
            )
            // 它什么时候写完我们不知道；先把盘上现有的读一遍，写好了这一下就看见。
            reloadMail()
            return reply != nil
        }
        return await runMailRound()
    }

    // MARK: - 请 being 跑一趟的公共节奏（邮局 / 账本共用）
    ...
    /// 再问。重问是安全的：账本那趟每次全量重送、天然幂等；邮局那趟请求里写明了
    /// 「投过的 draft_id 别再投」，我们这边回执也按 draft_id 去重。
...
#if os(macOS)
        // 账本就在本机，CLI 按同一个顺序解析同一个指针文件（BEING-RULES §一），
        // 单机还是关联都够得着。
        true
#else
    ...
    func nudgeLedgerRound() async -> Bool {
        ...
        // 但不告诉它有哪些，这条规矩它没法遵守——清单里只出现「已经属于某个项目」的那些名字。
    ...
    /// 说清楚这一趟发生了什么。被挡掉的必须说——「being 没写」和「写了被规矩 2 挡下」
    ...
    /// 手机单机时的一趟邮局：**这台设备自己跑完并落盘**。
    ///
    /// 手机够不着 Town，being 够不着手机的沙盒——中间没有共享文件系统，
    /// Loom 是唯一存在的那条线。所以取信、投递、合并全在这一个来回里做完，
    /// 不经 Mac、不经 iCloud（理由见 `BeingClient.mailRound`）。
    private func runMailRound() async -> Bool {
        // 占的是 `isAskingBeing`（机器那几趟），**不是 `isSending`**（人在说话）。
        // 以前占的是后者：这一趟要先「等 being 空出来」（最多 60 秒 ×3 轮），这几分钟里
        // 对话页的发送按钮全程是灰的，人打完一整段话按不动，也不知道为什么。
        // 账本那一趟（`nudgeLedgerRound`）本来就用的这个标志，两趟这下同形。
        guard !isAskingBeing else { return false }
        guard let connection else {
            sendError = "还没接入你的 being——去「我」那一屏粘一次 loom 链接。"
            notice = "还没接入你的 being，去「我」那一屏粘一次 loom 链接。"
            return false
        }
        isAskingBeing = true
        ...
        var outbox = mailStorage.readOutbox()
            ?? KairosMailOutbox(deviceID: KairosDevice.id, deviceName: KairosDevice.name)
        let waiting = KairosMailMerge.pending(mailbox, outbox: outbox)

        let client = BeingClient(connection: connection)
        let room = rooms.roundSession(KairosRoundKind.mail) // 邮局的话永远落这一间
        rooms.save()
        let trip = KairosRoundID.next(KairosRoundID.mailPrefix)
        let round: KairosMailRound
        do {
            round = try await withBeingTurn(client) {
                try await client.mailRound(
                    drafts: waiting,
                    round: trip,
                    scene: .round(KairosRoundKind.mail, being: beingName),
                    sessionId: room
                )
            }
        } catch BeingClientError.unreadableReply(let why) {
            connectionState = .online(beingName) // 同上：格式不对不是离线
            notice = why
            return false
        } catch {
            connectionState = .offline
            notice = "这趟邮局没跑成：\(error.localizedDescription)"
            return false
        }
        connectionState = .online(beingName)

        // 这一趟可能跑了几分钟（等 ta 空出来 + 想的时间）。这中间人类完全可能又写了一封信，
        // 或者撤回了一封——`sendMail` / `discardMailDraft` 都是读盘改盘，没被 `isSending` 挡着。
        // 拿开跑前那份内存态去写盘，就是拿旧的整份盖掉新的：刚写的那封信没了，
        // `lastSeq` 还会退回去，下一封复用同一个 seq，水位一推就把它也吞了。
        // 所以这里**重读一次盘**，往那份最新的上面剪。
        outbox = mailStorage.readOutbox() ?? outbox

        // 规矩 4 的形状不变：先落文件，再谈别的。回执先写——它决定水位，
        // 水位决定草稿能不能剪；顺序反了会把还没投出去的草稿剪掉。
        var next = KairosMailMerge.applyReceipts(mailbox, round.receipts, outbox: outbox)
        next = KairosMailMerge.applySync(
            next,
            received: round.received,
            sent: round.sent,
            beingID: round.beingID,
            beingName: round.beingName
        )
        guard mailStorage.writeMailbox(next) else {
            notice = "信取回来了，但没写下来。再试一次。"
            return false
        }
        mailbox = next

        let watermark = next.outboxWatermark[outbox.deviceID] ?? 0
        let before = outbox.drafts.count
        outbox.prune(sentThrough: watermark)
        if outbox.drafts.count != before { mailStorage.writeOutbox(outbox) }
        mailDrafts = outbox.drafts
        // 「跑成了」和「跑成了但有几处不对劲」是两回事，不说人就不知道。
        if !round.notes.isEmpty { notice = round.notes.joined(separator: "；") + "。" }
        return true
    }

    /// 把邮箱镜像并成单子上的消息行（两端都做）。
    ///
    /// 机械动作，不需要判断力：有对方、有原话，就是欠一个回复。规矩全在
    /// `KairosMessageRow.merging` 里（人手改过的不覆盖、删过的不再建、球权跟着
    /// 「谁最后说话」走）。为什么不等 being 消化完再建行：那条路上 being 睡着的时候
    /// Judy 来信就不出现，对每天回一百个人的人，漏一个人比看到一行没拟稿的裸行严重得多。
    ///
    /// **两端都做，但做的不一样，因为写法不一样**：
    ///
    ///   Mac    账本的写者，直接在锁里并整份——建新行、也跟着最新那封更新已有的行。
    ///   手机    只写自己的 outbox（多端写入那条），所以**只建新行，不动已有的行**。
    ///
    /// 手机为什么只建不改：outbox 并回账本时一律盖 human 戳（`KairosOutboxEntry.merged`
    /// 那句「从手机来的也是人改的」）。机械改出来的标题和状态盖上人戳，等于替人把这几个
    /// 字段对 being 锁死了（规矩 2）——那是假的人类决定。建新行没有这个问题：新建的
    /// `lastWriter` 是空的（`KairosItemEdit.created`），谁都能接着写。
    ///
    /// 手机为什么必须做：以前这一段是 Mac 专属，于是手机上一封信在 Mac 开机之前
    /// **根本不在单子上**——那正是手机得单独留一个 Inbox tab 的原因。而手机是主力设备，
    /// Mac 可能几天不开。2026-09-12 把 Inbox 并进单子，这一段就得跟着两端都有。
    /// 两边算出来的行 id 一样（`msg:<渠道>:<对方>`），谁先算都不会重。
    func ingestMessages() {
#if os(macOS)
        let threads = mailThreads
        guard !threads.isEmpty else { return }
        do {
            let committed = try KairosFiles.withLedgerTransaction(purpose: "ingest messages") { disk in
                guard let disk else { return nil }
                // 过一遍 normalized：清掉手工顺序里指向已不存在条目的 id，
                // 并把老格式的顺序键迁过来。
                // （原来这儿还有第二个理由——`activeItems` 按 `membership` 过滤，
                // 新建的行不补归属就看不见。那张表已撤，这条理由没了。）
                let merged = KairosWorkspaceEngine.normalized(
                    KairosMessageRow.merging(disk, threads: threads)
                )
                return merged == disk ? nil : merged
            }
            if let committed, committed != snapshot { apply(committed) }
        } catch {
            notice = "把信并成单子上的行没成：\(error.localizedDescription)"
        }
#else
        let threads = mailThreads
        guard !threads.isEmpty else { return }
        let known = Set(snapshot.items.map(\.id))
        let fresh = KairosMessageRow.merging(snapshot, threads: threads).items
            .filter { !known.contains($0.id) }
        guard !fresh.isEmpty else { return }
        var next = snapshot
        next.items.append(contentsOf: fresh)
        // `.edited` 而不是 `.created`：`.created` 会把**每一条**已有待办的 `lastWriter`
        // 清空（见 `stampHumanWriters`），人锁住的字段一次刷新全开了。这里只往后追加，
        // 已有的那些一个字段都没变，`.edited` 于是一个戳都不盖，新来的照样按新建处理。
        do { try commit(next, edit: .edited, createOnly: Set(fresh.map(\.id))) }
        catch { notice = "把信并成单子上的行没成：\(error.localizedDescription)" }
#endif
    }

    /// 写一封信。**不发**——只写进这台设备的草稿箱，being 下次醒来才真的发出去。
    /// 规矩 4 的形状没变：先落文件，再谈送达。
    ///
    /// 失败往外抛，**不写 store.notice**：notice 的弹窗挂在根视图上，
    /// 从写信 sheet 底下弹出来会把 sheet 整个顶掉，人刚写的那段字就没了
    /// 错误留给那一屏自己就地显示。
    func sendMail(to recipient: String, content: String) throws {
        let who = recipient.trimmingCharacters(in: .whitespacesAndNewlines)
        let body = content.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !who.isEmpty, !body.isEmpty else { throw KairosMailSendError.empty }

        var outbox = mailStorage.readOutbox()
            ?? KairosMailOutbox(deviceID: KairosDevice.id, deviceName: KairosDevice.name)
        outbox.append(recipient: who, content: body)
        guard mailStorage.writeOutbox(outbox) else { throw KairosMailSendError.writeFailed }
        mailDrafts = outbox.drafts
    }

    /// 撤回一封 being 还没拿走的信。拿走之后就撤不了了——那封信已经在路上。
    func discardMailDraft(_ draftID: String) {
        guard var outbox = mailStorage.readOutbox() else { return }
        outbox.discard(draftID)
        guard mailStorage.writeOutbox(outbox) else { return }
        mailDrafts = outbox.drafts
    }

    /// 打开一条对话就算读了。没有未读就不写盘——省掉的不是那点 IO，
    /// 是「每次点开都动一次文件」在 iCloud 上引起的无谓同步。
    func markMailThreadRead(_ correspondent: String) {
        let unread = mailbox.received.contains {
            $0.sender == correspondent && mailRead.isUnread($0, from: correspondent)
        }
        guard unread else { return }
        mailRead.markRead(correspondent)
        mailRead.save()
    }

    /// 信的「了结」：和待办同一个词、同一个意思——到此为止，从单子上下去。
    /// 不是删掉，也不是拉黑这个人：对方再来信就自己回到单子上（`KairosMailReadCursor.isDone`）。
    func markMailThreadDone(_ correspondent: String) {
        mailRead.markDone(correspondent)
        mailRead.save()
    }

    func reopenMailThread(_ correspondent: String) {
        mailRead.reopen(correspondent)
        mailRead.save()
    }
    ...
    /// 规矩 4 的固定顺序：**账本已经落盘了**才走到这里，攒一条 routine 通知进房间。
    ///
    /// 为什么是 routine：这是人自己刚做的事，回头亮给自己看是噪音。
    /// ask 级留给 being 反过来要人拍板的时候（规矩 5）。
    ///
    ...
    /// 「事项亮提示」只亮 ask 级（v2.3 §四 + 规矩 5）：全是 routine 的不动视觉状态，
    ...
    /// 规矩 4 的第二步。**调用它之前账本必须已经落盘成功**——这个顺序是固定的，
    ...
    ///     不给就是一次性的——`nudge` 那几条（叫 being 去邮局 / 看一眼账本）走的是这条，
    ///     它们只要一个结果，不需要留痕。
    ///
    ...
    func speak(_ text: String, in item: KairosItem?, deskRoom: String? = nil) async -> BeingReply? {
        ...
        if let item {
            // 人说话 = 新的用户信号 → 解开这条待办的人类锁（规矩 2 后半句）。
            // **只有待办有锁**，桌面对话没有可解的。
            releaseHumanLocks(for: item.id)
        }
        if let roomKey {
            // 先记成「没发出去」，**服务端一收下就翻成已送达**（`.accepted`）——不是等
            // 整段回复收完。那可能是几分钟以后，这几分钟里气泡挂着红字说没发出去，
            // 而 being 早就在答了——明明已经发出去了。
        ...
        do {
            let itemID = item?.id
            let reply = try await BeingClient(connection: connection).speak(
                outgoing,
                // 桌面对话没有「哪一条」，`scene(for: nil)` 给的正是 `.main`——
                // 和 loom 里那个主对话同一个场景，本来就对。
                scene: scene(for: itemID),
            ...
            if reply.spliced {
                notice = "消息已送达，Being 正在思考中，稍后再看"
            }
            if let roomKey, !reply.text.isEmpty {
            ...
            if trace.accepted, let item {
                ...
                streamRecovery[item.id] = Task { [weak self] in
                    await self?.resumeStream(trace, in: item)
                }
                return nil
            }
            sendError = error.localizedDescription
            // 有房间就有地方就地显示原因（房间自己画 `sendError`）；
            // 连房间都没有的那几条（nudge）才弹一下。
            if roomKey == nil { notice = "发送失败：\(error.localizedDescription)" }
            return nil
        }
    }
    ...
    private func resumeStream(_ trace: BeingStreamTrace, in item: KairosItem) async {
        guard let connection else { return }
        defer { streamRecovery.removeValue(forKey: item.id) }
        // 断的是我们这头的连接，不是 being 那口气。气泡留着，它接着往下写。
        liveReply[item.id] = liveReply[item.id] ?? ""
        let client = BeingClient(connection: connection)
        ...
        while !finished, !Task.isCancelled, Date() < deadline {
        ...
        if !reader.text.isEmpty {
            ...
            rooms.save()
            if !finished {
                // 没等到那口气收尾就断了。宁可说一句，也别让人把半截当整段——
                // 完整的那份在 being 自己的历史里，loom 打开就是。
                noteInRoom(item.id, text: "上面这段可能没收全，完整的在 loom 里。")
            }
        } else {
    ...
    /// 没发出去的那句再发一次：先把旧气泡撤掉，再当成新的一句说。
    func resend(_ message: KairosRoomMessage, in item: KairosItem) async {
        guard message.origin == KairosMessageOrigin.user, !message.delivered else { return }
    ...
    private var beingRulesConnectionKey: String? {
    ...
    private func beingRulesDefaultsKey(_ prefix: String) -> String? {
        guard let id = beingRulesConnectionKey else { return nil }
        return "\(prefix).\(id)"
    }
    ...
    /// 规矩全文到了才算「已收下」：光提 `BEING-RULES` 五个字不算——being 随口一提就会误记成已送达。
    ...
    /// 最近一封像规矩全文的来信正文（设置页「看规矩全文」用）。
    var beingRulesText: String? {
        mailbox.received
            .filter { $0.content.localizedCaseInsensitiveContains("BEING-RULES") && $0.content.count >= 500 }
            .max { $0.content.count < $1.content.count }?
            .content
    }

    private func scanIncomingMessagesForBeingRules() {
        for message in mailbox.received {
            noteBeingRulesDelivery(in: message.content)
        }
    }

    /// 向 being 房间发结构化请求，请它把 BEING-RULES 全文发回来。
    func requestBeingRules() async {
        guard connection != nil else { return }
        let prompt = "请把你的 BEING-RULES 全文发到这个房间，一次就够。"
        _ = await speak(prompt, in: nil, deskRoom: Self.beingRulesDeskRoom)
        writeBeingRulesStamp(prefix: "rulesRequestedAt", date: Date())
    }

    func loadSnapshot() {
        // 邮箱和账本住同一个文件夹，一起读。分开的话就会出现这一幕：
        // 手机上刚选完 iCloud 文件夹，待办立刻出来了，Inbox 还固执地说「邮局还没接通」——
        // 因为选文件夹只触发了这里，而邮箱是在 init 里读过一次就不再读了。
        defer { reloadMail() }
...
#if os(macOS)
    /// 把手机等设备的 outbox 并进账本。锁内做，和别的写一样受规矩 1 约束。
