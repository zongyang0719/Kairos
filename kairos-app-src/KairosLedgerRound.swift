...
///
/// 账本那趟（`KairosLedgerRoundReply`）用它抠块：
/// 对面是个会客套、会换格式的 agent，不是 RPC 端点——所以抠块要容错，
/// 抠错的后果是整趟白跑。
...
struct KairosLedgerRoundItem: Encodable, Hashable {
    ...
    var ask: String
    /// 谁在等他回。**只有消息行才有**，空的就不发。
    /// 少了它，「私信你不用建，你要做的是补选项」这条规则在这条路上没法执行——
...
struct KairosLedgerPatch: Decodable, Hashable {
    ...
    /// 选项的形状。**和 Node 侧 `ledger/store.js` 的 `isValidOptions` 同一条规则**：
    ...
    static func rejection(_ field: String, value: String, projects: [String]) -> String? {
        ...
        case "status":
            guard KairosStatus.all.contains(value) else {
                return "状态只认 " + KairosStatus.all.joined(separator: " / ")
            }
            // 证据闸，与 CLI 那条路（ledger/cli.js）同一道：可逆的全自动，不可逆的拿证据。
            // 本通道带不了 evidence（它的入口是人的锁与 CLI），所以 closed 在这里一律顶回，
            // 指两条同款出路：让人在卡上点了结，或走 CLI 带 evidence；证据不够要人拍板的，
            // 把判断写进 options 让人点。
            if value == KairosStatus.closed {
                return "了结要有证据，这条通道带不了。确实完成了：让人在卡上点，或走 CLI 带 evidence；"
                    + "要他拍板：把判断写进 options 让人点"
            }
            return nil
        case "source":
...
enum KairosLedgerRoundReply {
    ...
    ///
    /// 规则（协议长什么样、值集、短号只在本趟有效、清空要显式）写在
    /// `BEING-RULES.md` 的「直连 bap.ledgerround/1」那节，一次性的东西不该每趟重发。
    /// 骨架留着是 2026-08-29 的裁决：多花几十 token，省掉一次带工具调用的全量失败往返。
    /// 它没读操作卡也不会出事——拒收是失败安全的，下一趟 `last_round` 会把那条规则贴回它脸上。
    ///
    ...
    static func request(
        ...
        var lines = [
            ...
            "规则在 BEING-RULES.md 的「直连 bap.ledgerround/1」那节，这里只说这一趟的事。",
            "",
            "本趟编号 \(round)",
        ]
        if !projects.isEmpty {
    ...
    /// 上一趟的判决。**这是规则按需重述的地方**：不犯就不说，犯了才把那一条贴回去。
    ...
    ///   原样贴回去就把围栏配对搅乱了（整趟白跑）。
    /// - **换行**：判决是一行一条的列表，中间断行会让后面那半截看着像新的一条规则。
    ...
    static func parse(_ text: String, round: String, correlated: Bool = false) throws -> Reply {
        ...
        // 于是 being 答非所问、或者把别的门牌的块答进来，屏幕上写的是「这一轮没有要改的」——
        // 它压根没答，人看到的是它答了「不用改」。
...
enum KairosLedgerRoundMerge {
    /// 把 being 提的改动并进账本。
    ///
    /// **规则 2 在这里落地**：人类手改过的字段（`lastWriter[字段] == human`）一律跳过。
