...
struct KairosOption: Codable, Hashable, Identifiable {
    ...
    /// **缺字段不能整条读不出来。** 和 `KairosCounterpart` / `KairosUtterance` 同一条规则，
...
struct KairosItem: Codable, Hashable, Identifiable {
    ...
    /// 规则 2（v2.3 §二）：字段级写者标记 `字段名 -> human | being`。
...
/// 规则 2 按字段名比对写者，名字对不上等于保护没生效，而且不会报错，只会安静地
/// 让 being 盖掉人刚改的东西。
enum KairosField {
...
extension KairosItem {
    /// 把 `other` 的指定业务字段搬到自己身上。规则 1 的 rebase 用它：以盘上最新那条
...
enum KairosItemEdit {
    ...
    /// 不盖 `human`——自动写入永远不该假冒成人做的决定，否则规则 2 从此形同虚设。
    case beingWrite
