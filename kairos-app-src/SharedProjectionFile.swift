...
struct SharedLedgerFolder {
    ...
    func adopt(_ url: URL) throws {
        ...
        let carriedMail = readMailOutbox(deviceID: device)
        ...
        if let carriedMail, !carriedMail.drafts.isEmpty, readMailOutbox(deviceID: device) == nil {
            writeMailOutbox(carriedMail)
        }
    }
    ...
    // MARK: - 邮箱（同一个文件夹，同一条规矩：镜像只读，自己的草稿箱只有自己写）

    /// being 写的 Town 邮箱镜像。手机只读——它连 Town 都够不着，更没资格改镜像。
    func readMailbox() -> KairosMailbox? {
        withFolder { folder in
            let url = folder.appendingPathComponent(KairosMailFiles.mailboxName)
            guard FileManager.default.fileExists(atPath: url.path) else { return nil }
            if !KairosFiles.isMaterialized(url) {
                try? FileManager.default.startDownloadingUbiquitousItem(at: url)
                return nil
            ...
            Self.requestLatestVersion(url)
            var mailbox: KairosMailbox?
            NSFileCoordinator().coordinate(readingItemAt: url, options: [], error: nil) { readURL in
                guard let data = try? Data(contentsOf: readURL) else { return }
                mailbox = try? JSONDecoder().decode(KairosMailbox.self, from: data)
            }
            return mailbox?.isSupportedProtocol == true ? mailbox : nil
        } ?? nil
    }

    func readMailOutbox(deviceID: String) -> KairosMailOutbox? {
        withFolder { folder in
            let url = KairosMailFiles.outboxFile(in: folder, deviceID: deviceID)
            guard FileManager.default.fileExists(atPath: url.path) else { return nil }
            if !KairosFiles.isMaterialized(url) {
                try? FileManager.default.startDownloadingUbiquitousItem(at: url)
                return nil
            }
            var outbox: KairosMailOutbox?
            NSFileCoordinator().coordinate(readingItemAt: url, options: [], error: nil) { readURL in
                guard let data = try? Data(contentsOf: readURL) else { return }
                outbox = try? JSONDecoder().decode(KairosMailOutbox.self, from: data)
            }
            return outbox
        } ?? nil
    }
    ...
    @discardableResult
    func writeMailOutbox(_ outbox: KairosMailOutbox) -> Bool {
        (withFolder { folder -> Bool in
            let directory = folder.appendingPathComponent(
                KairosMailFiles.outboxFolderName, isDirectory: true
            )
            try? FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
            let url = KairosMailFiles.outboxFile(in: folder, deviceID: outbox.deviceID)
            let encoder = JSONEncoder()
            encoder.outputFormatting = [.prettyPrinted, .sortedKeys, .withoutEscapingSlashes]
            guard let data = try? encoder.encode(outbox) else { return false }
            var ok = false
