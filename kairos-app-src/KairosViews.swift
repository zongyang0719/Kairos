...
struct KairosSettingsView: View {
    ...
    private var beingRulesConfirmBody: Text {
        var text = AttributedString("点「发送请求」，Kairos 会让 being 去读")
        var link = AttributedString("规则文档")
        let rulesFileURL = KairosFiles.dataDirectory.appendingPathComponent("BEING-RULES.md")
        if FileManager.default.fileExists(atPath: rulesFileURL.path) {
            link.link = URL(fileURLWithPath: rulesFileURL.path)
        }
        text += link
