...
/// 一行是从哪来的：账本上的 `source`（`KairosSource`）。邮局镜像里的信算 `inbox`。
...
struct KairosMacFilter: Equatable, Codable {
    ...
    /// 信没有自己的优先级和项目：一律算 P2、不属于任何项目；状态按它现在的样子推。
    func allows(_ thread: KairosMailThread) -> Bool {
        guard allows(source: KairosSource.inbox) else { return false }
        if project != nil { return false }
        if !tiers.isEmpty, !tiers.contains(thread.derivedTier) { return false }
        if !statuses.isEmpty, !statuses.contains(thread.derivedStatus) { return false }
        return true
    }
}

extension KairosMailThread {
    /// 信在优先级组里的位置：一律 P2。being 要是觉得某封要紧，会建一条待办说这件事。
    var derivedTier: String { "P2" }

    /// 信在状态组里的位置：没读的 = 未开始（要你看）；有一封等 being 发出 = 待定；来往中 = 进行中。
    var derivedStatus: String {
        if hasUnread { return KairosStatus.todo }
        if hasPending { return KairosStatus.pending }
        return KairosStatus.doing
    }
}
...
final class KairosMacShell: ObservableObject {
    ...
    @Published var selectedCorrespondent: String?
    ...
    var inspectorShown: Bool { selectedCorrespondent != nil || roomShown }
...
struct KairosWindow: View {
    ...
    private var inspectorBinding: Binding<Bool> {
        Binding(
            ...
            get: { shell.selectedCorrespondent != nil || (shell.roomShown && store.selectedItem != nil) },
            set: { shown in
    ...
    private var inspectorContent: some View {
        if let correspondent = shell.selectedCorrespondent {
            MailThreadScreen(store: store, correspondent: correspondent)
                .id(correspondent)
        } else if let item = store.selectedItem, shell.roomShown {
...
/// 间距也按工具栏的规矩排；`.plain` 会把这些一起抹掉，点上去像点在一张图上。
...
private struct FilterChip: View {
    ...
    var body: some View {
        Button {
            ...
            .foregroundStyle(on ? KairosMacPalette.onDone : Color.secondary)
            .padding(.horizontal, 9)
            .padding(.vertical, 4)
            .background(on ? KairosMacPalette.doneFill : Color.primary.opacity(0.06), in: Capsule())
...
private struct SaveViewSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text(editing == nil ? "存为视图" : "改视图").font(.headline)
...
private struct SymbolPicker: View {
    ...
    private let columns = Array(repeating: GridItem(.fixed(28), spacing: 6), count: 8)
...
private struct ProjectSidebar: View {
    ...
    var body: some View {
        ...
        // 「新建」搬到单子头上那颗橙色 +（`NewItemButton`，⌘N 同一件事），「去邮局」整个功能撤了。
    ...
    /// 边栏底下这一格**只放设置**。
    ///
    /// 「新建」「新建项目」「去邮局」都上顶上去了——常用的动作放在手够得着的地方，
    /// 窗口底下那一格留给一年点两次的东西。
...
private struct AllItemsRow: View {
    ...
    var body: some View {
        Label("全部", systemImage: "tray.full")
            ...
            .animation(.smooth(duration: 0.14), value: targeted)
    }
}
    ...
    var body: some View {
        Label(project, systemImage: store.macProjectSymbol(project))
            ...
            .dropDestination(for: KairosProjectDragPayload.self) { payloads, _ in
                ...
                withAnimation(.smooth(duration: 0.2)) { store.macReorderProjects(names) }
                return true
            ...
            .animation(.smooth(duration: 0.14), value: targeted)
            .animation(.smooth(duration: 0.14), value: reordering)
            .contextMenu {
...
struct KairosMacGroup: Identifiable {
    ...
    let title: String
    let color: Color
    /// **单子上只有一种行**。以前这里是 `[KairosMacRow]`——一个两个 case 的
    /// enum，待办是账本条目、信是从 `mailbox.json` 现合成的。那种形状下信不能拖、不能改档位、
    /// 不能进项目（档位是显示层硬塞的 P2），而人类的第一条是「所有行必须一样，
    /// 有一种行不能勾，肌肉记忆就断了」。现在信也是账本条目（`KairosMessageRow`），
    /// 一行就是一条 item，消息只是**带对方**的那种。
    let rows: [KairosItem]
...
private struct MainScreen: View {
    ...
    var body: some View {
        ...
        .animation(.snappy(duration: 0.22), value: shell.composing)
        .toolbar { toolbar }
...
///
/// 它上一版一句话两个出口：回车记成待办、⌘回车「问 Soul」，问完右侧开出一块桌面对话。
...
struct CaptureBar: View {
    ...
    var body: some View {
        ...
        .animation(.snappy(duration: 0.18), value: trimmed.isEmpty)
        .animation(.snappy(duration: 0.18), value: justAdded)
        .onAppear {
...
private struct GroupedList: View {
    ...
    var body: some View {
        ...
        .animation(.smooth(duration: 0.24), value: groups.map { $0.rows.map(\.id) })
    }
...
private struct DraggableItemRow: View {
    ...
    var body: some View {
        ...
        .animation(.smooth(duration: 0.14), value: targeted)
        .animation(.smooth(duration: 0.14), value: shell.draggingIDs)
        .contextMenu {
...
private struct ItemRow: View {
    ...
    var body: some View {
        ...
        .animation(.smooth(duration: 0.2), value: completing)
        .contentShape(.rect)
    ...
    private var trailing: some View {
        HStack(spacing: 10) {
            ...
            } else if !isClosed, store.hasUnreadAsk(item.id) {
                Circle()
                    .fill(KairosMacPalette.attention)
                    .frame(width: 8, height: 8)
                    .accessibilityLabel("\(store.beingNameLeading)在等你")
            }
    ...
    var body: some View {
        ZStack {
            if closed {
                ...
                // 和 P1 的橙在一屏上打架。
                // 规矩定死：**语义色归 app（橙 / 红），选中和焦点归系统**。
                Circle()
                    .fill(KairosMacPalette.doneFill)
                Image(systemName: "checkmark")
                    .font(.system(size: size * 0.5, weight: .bold))
                    .foregroundStyle(KairosMacPalette.onDone)
            } else {
        ...
        .animation(.spring(response: 0.3, dampingFraction: 0.7), value: closed)
    }
}
...
struct TierTag: View {
    ...
    var body: some View {
        ...
        Text(value)
            ...
            .foregroundStyle(value == "P0" || value == "P1" ? KairosMacTier.color(value) : Color.secondary)
    }
}
...
struct KairosSettingsView: View {
    ...
    var body: some View {
        Form {
            Section("Being") {
                ...
                TextField("名字", text: $name, prompt: Text("比如 Soul"))
                ...
                if saved { Label("已保存", systemImage: "checkmark.circle.fill").foregroundStyle(.green) }
    ...
    /// 探测成功或已经在线——才露出「规矩」递送（纯手账本没连 being 时不显示）。
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let delivered = store.beingRulesDeliveredAt {
                Text("已收下 · \(beingRulesDate(delivered))")
                    .font(.callout)
                if let rulesText = store.beingRulesText {
                    DisclosureGroup("看规矩全文") {
                        ScrollView {
                            Text(rulesText)
                                .font(.caption)
                                .frame(maxWidth: .infinity, alignment: .leading)
                ...
                Text("连上之后，可以让 being 把规矩全文发到这个房间，你这边记「已收下」。")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Button(requestingBeingRules ? "正在发…" : "向 \(beingDisplayName) 请求规矩") {
                    Task { await requestBeingRulesFromSettings() }
                }
                .disabled(requestingBeingRules)
            }
...
extension KairosStore {
    ...
    func macSelectionBinding(_ shell: KairosMacShell) -> Binding<Set<String>> {
        Binding(
            ...
            set: { ids in
                shell.selectedCorrespondent = nil
    ...
    /// 搜信：人名或最新一句里带这个词的。
    func macSearchMail(_ query: String) -> [KairosMailThread] {
        let needle = query.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard !needle.isEmpty else { return [] }
        return mailThreads.filter { thread in
            thread.correspondent.lowercased().contains(needle)
                || thread.entries.contains { $0.content.lowercased().contains(needle) }
        }
    }
    ...
    func macMove(_ item: KairosItem, by offset: Int, in key: KairosMacGroup.Key, filter: KairosMacFilter) {
        ...
        withAnimation(.smooth(duration: 0.24)) { macReorder(group: section) }
    }
...
enum KairosMacPalette {
    ...
    /// 现在两端一份数值、一套规矩：**跟着深浅模式换一档，压上去的前景色跟着翻**（`onAccent`）——
    ...
    /// 看板一列：比窗口底色深一度。
    static let column = Color.primary.opacity(0.045)
}
