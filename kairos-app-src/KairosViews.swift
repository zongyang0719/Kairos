...
struct KairosWindow: View {
    ...
    private var inspectorBinding: Binding<Bool> {
        Binding(
            ...
            get: { shell.roomShown && store.selectedItem != nil },
            set: { shown in
...
struct KairosSettingsView: View {
    ...
    @State private var rulesPreviewText: String?
    ...
    private var beingRulesDeliveryBlock: some View {
        VStack(alignment: .leading, spacing: T.Spacing.s) {
            ...
            if let delivered = store.beingRulesDeliveredAt {
                ...
                Text("点一下，以你的名义给它发一条消息，把规则全文要过来；不经你点，不会发。")
                    .font(.system(size: T.TypeScale.caption))
                    .foregroundStyle(T.Ink.secondary)
    ...
    private func openRulesRequestConfirm() {
        rulesPreviewText = loadRulesPreview()
        showingRulesRequestConfirm = true
    }

    /// 从 app 内置的 BEING-RULES.md 提取简化预览：核心句 + 每条规则的名字。
    /// 全文以 being 实际发回的为准，这里只让人点之前知道规则大概是什么。
    private func loadRulesPreview() -> String? {
        guard let url = Bundle.main.url(forResource: "BEING-RULES", withExtension: "md"),
        ...
        for rawLine in raw.components(separatedBy: "\n") {
            ...
            if inRules, line.first?.isNumber == true,
               let dot = line.firstIndex(of: ".") {
                let body = line[line.index(after: dot)...].trimmingCharacters(in: .whitespaces)
                let name: Substring
                if body.hasPrefix("**"), let end = body.dropFirst(2).firstIndex(of: "*") {
                    name = body.dropFirst(2)[..<end]
                } else {
                    name = body.prefix(24)
                }
                if !name.isEmpty { items.append("· \(name)") }
            } else if !inRules && preamble.isEmpty && line.hasPrefix("**") && line.hasSuffix("**") {
                preamble = String(line.dropFirst(2).dropLast(2))
            }
        }
        var lines: [String] = []
        if !title.isEmpty { lines.append(title) }
        if !preamble.isEmpty { lines.append(preamble) }
        if !items.isEmpty {
            lines.append("")
            lines.append(contentsOf: items)
            lines.append("（共 \(items.count) 条；以上是预览，全文以 being 发回的为准）")
        }
    ...
    private var rulesRequestPreviewSheet: some View {
        VStack(alignment: .leading, spacing: T.Spacing.m) {
            Text("发一条规则请求")
                .font(.system(size: T.TypeScale.headline, weight: .semibold))
            Text("点「发送请求」后，Kairos 会替你向 \(beingDisplayName) 发一条消息，请它把规则全文发回来。不经你确认，不会发。")
                .font(.system(size: T.TypeScale.caption))
                .foregroundStyle(T.Ink.secondary)
            if let preview = rulesPreviewText {
                ScrollView {
                    Text(preview)
                        .font(.system(size: T.TypeScale.caption))
                        .lineSpacing(T.TypeScale.captionLineSpacing)
