...
struct KairosSettingsView: View {
    ...
    /// 直接显示 app 内置的 BEING-RULES.md 全文——不做摘要，人看到的就是将要请求的东西。
    ...
    private var rulesRequestPreviewSheet: some View {
        VStack(alignment: .leading, spacing: T.Spacing.m) {
            Text("发一条规则请求")
                .font(.system(size: T.TypeScale.headline, weight: .semibold))
            Text("BEING-RULES 是 Kairos 与 being 的工作约定：账本怎么记、增删改谁负责、事情怎么流转。点「发送请求」后，Kairos 会把这份文档放进账本文件夹，替你请 \(beingDisplayName) 去读它、按它来——文档全文不进聊天，只过一句话。不经你确认，不会发。")
                .font(.system(size: T.TypeScale.caption))
                .foregroundStyle(T.Ink.secondary)
